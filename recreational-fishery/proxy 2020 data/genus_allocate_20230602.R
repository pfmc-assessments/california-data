...
# Julia Coates, May 2023
# This script is revised from the one prepared in February 2023. It generates a table that pulls original mortality estimates for copper, black, and canary rockfish by mode and district in both numbers of fish and kg.  The table also shows "allocated" values by species, mode, and district.  These allocated values are to be ADDED to the original values to generate revised species specific mortality estimates for covid years when a high proportion of CRFS samples were in the "rockfish genus" category.  In the previous table from February, the values in the allocated section of the table were meant to REPLACE the original values from RecFIN.  
...
allrockpr <- filter(allrock, RECFIN_MODE_NAME!="PARTY/CHARTER BOATS")  # This groups shore-based modes in with PR.
...
sumave1819 <- sum(pr$ave1819)  # The total average catch in 2018-19 
...
sumave1819b <- sum(pr$ave1819b)  # The total average catch in 2018-19 without rockfish genus
...
pcmat2020 <- matrix(NA, 12, 3)
colnames(pcmat2020) <- c("copper_num", "black_num", "canary_num")
...
for (i in 1:length(districts)){
  ...
  pc[is.na(pc)] <- 0
  pc$ave1819 <- (pc$x2018 + pc$x2019)/2
  sumave1819 <- sum(pc$ave1819)  # The total average catch in 2018-19 
  pc$ave1819b <- pc$ave1819  # Copy this average to a new column
  pc[pc$species=="ROCKFISH GENUS", "ave1819b"] <- 0 # Remove rockfish genus catch from this new column
  sumave1819b <- sum(pc$ave1819b)  # The total average catch in 2018-19 without rockfish genus 
  pc$prop1819 <- pc$ave1819 / sumave1819  # Calculate the proportion of catch for each species to the total
  pc$prop1819b <- pc$ave1819b / sumave1819b  # Calculate the proportion of catch for each species to the total without rockfish genus
  sum2020 <- sum(pc$x2020) # Get the total catch in 2020 including rockfish genus
  ...
  pc$x2020allocated <- (pc$prop1819b * to_allocate[1,1]) # Calculate just the allocated number of fish, don't add to original speciated number
  pc[pc$species=="ROCKFISH GENUS", "x2020allocated"] <- exp_rg_2020  # Replace the rockfish genus catch with the expected value
  sum(pc$x2020allocated)  # Check that the new allocated 2020 sum is equal to the original amount of rockfish genus in 2020.  It is.
  pc <- as.data.frame(pc)
  pcmat2020[i,1] <- ifelse(length(pc[pc$species=="COPPER ROCKFISH", "x2020"])==0, 0, pc[pc$species=="COPPER ROCKFISH", "x2020"])
  pcmat2020[i,2] <- ifelse(length(pc[pc$species=="BLACK ROCKFISH", "x2020"])==0, 0, pc[pc$species=="BLACK ROCKFISH", "x2020"])
  pcmat2020[i,3] <- ifelse(length(pc[pc$species=="CANARY ROCKFISH", "x2020"])==0, 0, pc[pc$species=="CANARY ROCKFISH", "x2020"])
  pcmat2020[i+6,1] <- ifelse(length(pc[pc$species=="COPPER ROCKFISH", "x2020allocated"])==0, 0, pc[pc$species=="COPPER ROCKFISH", "x2020allocated"])
  pcmat2020[i+6,2] <- ifelse(length(pc[pc$species=="BLACK ROCKFISH", "x2020allocated"])==0, 0, pc[pc$species=="BLACK ROCKFISH", "x2020allocated"])
  pcmat2020[i+6,3] <- ifelse(length(pc[pc$species=="CANARY ROCKFISH", "x2020allocated"])==0, 0, pc[pc$species=="CANARY ROCKFISH", "x2020allocated"])
}

pcmat2020 <- as.data.frame(round(pcmat2020, 2))

write.csv(pcmat2020, "genus_pc_2020_20230530.csv")

#================================================================================================
# Create a matrix of allocated values by district for PC in 2021

pcmat2021 <- matrix(NA, 12, 3)
colnames(pcmat2021) <- c("copper_num", "black_num", "canary_num")
...
for (i in 1:length(districts)){
  ...
  to_allocate <- (pc[pc$species=="ROCKFISH GENUS", "x2021"]) - exp_rg_2021  # This is the amount of rockfish genus catch in 2021 that needs to be allocated
  #pc$x2021allocated <- pc$x2021 + (pc$prop1819b * to_allocate[1,1]) # Allocate the extra rockfish genus catch
  pc$x2021allocated <- (pc$prop1819b * to_allocate[1,1]) # Calculate just the allocated number of fish, don't add to original speciated number
  pc[pc$species=="ROCKFISH GENUS", "x2021allocated"] <- exp_rg_2021  # Replace the rockfish genus catch with the expected value
  sum(pc$x2021allocated)  
  pc <- as.data.frame(pc)
  pcmat2021[i,1] <- ifelse(length(pc[pc$species=="COPPER ROCKFISH", "x2021"])==0, 0, pc[pc$species=="COPPER ROCKFISH", "x2021"])
  pcmat2021[i,2] <- ifelse(length(pc[pc$species=="BLACK ROCKFISH", "x2021"])==0, 0, pc[pc$species=="BLACK ROCKFISH", "x2021"])
  pcmat2021[i,3] <- ifelse(length(pc[pc$species=="CANARY ROCKFISH", "x2021"])==0, 0, pc[pc$species=="CANARY ROCKFISH", "x2021"])
  pcmat2021[i+6,1] <- ifelse(length(pc[pc$species=="COPPER ROCKFISH", "x2021allocated"])==0, 0, pc[pc$species=="COPPER ROCKFISH", "x2021allocated"])
  pcmat2021[i+6,2] <- ifelse(length(pc[pc$species=="BLACK ROCKFISH", "x2021allocated"])==0, 0, pc[pc$species=="BLACK ROCKFISH", "x2021allocated"])
  pcmat2021[i+6,3] <- ifelse(length(pc[pc$species=="CANARY ROCKFISH", "x2021allocated"])==0, 0, pc[pc$species=="CANARY ROCKFISH", "x2021allocated"])
}

pcmat2021 <- as.data.frame(round(pcmat2021, 2))

write.csv(pcmat2021, "genus_pc_2021_20230530.csv")

#================================================================================================
# Create a matrix of allocated values by district for PR in 2020

prmat <- matrix(NA, 12, 3)
colnames(prmat) <- c("copper_num", "black_num", "canary_num")
...
for (i in 1:length(districts)){
  ...
  pr$x2020allocated <- (pr$prop1819b * to_allocate[1,1]) # Calculate just the allocated number of fish, don't add to original
  pr[pr$species=="ROCKFISH GENUS", "x2020allocated"] <- exp_rg_2020  # Replace the rockfish genus catch with the expected value
  sum(pr$x2020allocated)  # Check that the new allocated 2020 sum is equal to the original 2020 sum.  It is.
  pr <- as.data.frame(pr)
  prmat[i,1] <- ifelse(length(pr[pr$species=="COPPER ROCKFISH", "x2020"])==0, 0, pr[pr$species=="COPPER ROCKFISH", "x2020"])
  prmat[i,2] <- ifelse(length(pr[pr$species=="BLACK ROCKFISH", "x2020"])==0, 0, pr[pr$species=="BLACK ROCKFISH", "x2020"])
  prmat[i,3] <- ifelse(length(pr[pr$species=="CANARY ROCKFISH", "x2020"])==0, 0, pr[pr$species=="CANARY ROCKFISH", "x2020"])
  prmat[i+6,1] <- ifelse(length(pr[pr$species=="COPPER ROCKFISH", "x2020allocated"])==0, 0, pr[pr$species=="COPPER ROCKFISH", "x2020allocated"])
  prmat[i+6,2] <- ifelse(length(pr[pr$species=="BLACK ROCKFISH", "x2020allocated"])==0, 0, pr[pr$species=="BLACK ROCKFISH", "x2020allocated"])
  prmat[i+6,3] <- ifelse(length(pr[pr$species=="CANARY ROCKFISH", "x2020allocated"])==0, 0, pr[pr$species=="CANARY ROCKFISH", "x2020allocated"])
}

prmat <- as.data.frame(round(prmat, 2))

write.csv(prmat, "genus_pr_2020_20230530.csv")

#=========================================================================================
# Load in CRFS data on average weight to convert matrices of allocated fish to weight.
# These average weight tables are created by using the files CRFS PC Catch and Effort Estimates DDM (RecFIN format)_All_2019 (06-03-2020).xlsx and PRTotalMortalityAnnualEstimage_2019_(6_3_2020)_H-Factor.xlsx.  These data are from 2019 when sampling rates were higher.  I used pivot tables to calculate the average weight within a district.  I wanted to do this in R but the formatting of the original file imported in strange ways and was difficult to clean up.
...
genus_allocate$district <- rep(1:6, 6)

# Calculate and fill in the original mortality estimates by weight before allocation
pc20 <- filter(allrockpc, RECFIN_YEAR==2020)
pc20 <- filter(pc20, SPECIES_NAME %in% c("COPPER ROCKFISH", "BLACK ROCKFISH", "CANARY ROCKFISH"))
pc20_district_kg <- pc20 %>% group_by(SPECIES_NAME, DISTRICT_NAME) %>% summarize(sumcatchkg=sum(TOTAL_MORTALITY_KG))
pc20_district_kg <- spread(pc20_district_kg, SPECIES_NAME, sumcatchkg)
pc20_district_kg[is.na(pc20_district_kg)] <- 0
pc20_district_kg <- pc20_district_kg[match(districts, pc20_district_kg$DISTRICT_NAME),]
pc20_district_kg <- pc20_district_kg[, c("DISTRICT_NAME", "COPPER ROCKFISH", "BLACK ROCKFISH", "CANARY ROCKFISH")]

pc21 <- filter(allrockpc, RECFIN_YEAR==2021)
pc21 <- filter(pc21, SPECIES_NAME %in% c("COPPER ROCKFISH", "BLACK ROCKFISH", "CANARY ROCKFISH"))
pc21_district_kg <- pc21 %>% group_by(SPECIES_NAME, DISTRICT_NAME) %>% summarize(sumcatchkg=sum(TOTAL_MORTALITY_KG))
pc21_district_kg <- spread(pc21_district_kg, SPECIES_NAME, sumcatchkg)
pc21_district_kg[is.na(pc21_district_kg)] <- 0
pc21_district_kg <- pc21_district_kg[match(districts, pc21_district_kg$DISTRICT_NAME),]
pc21_district_kg <- pc21_district_kg[, c("DISTRICT_NAME", "COPPER ROCKFISH", "BLACK ROCKFISH", "CANARY ROCKFISH")]

pr20 <- filter(allrockpr, RECFIN_YEAR==2020)
pr20 <- filter(pr20, SPECIES_NAME %in% c("COPPER ROCKFISH", "BLACK ROCKFISH", "CANARY ROCKFISH"))
pr20_district_kg <- pr20 %>% group_by(SPECIES_NAME, DISTRICT_NAME) %>% summarize(sumcatchkg=sum(TOTAL_MORTALITY_KG))
pr20_district_kg <- spread(pr20_district_kg, SPECIES_NAME, sumcatchkg)
pr20_district_kg[is.na(pr20_district_kg)] <- 0
pr20_district_kg <- pr20_district_kg[match(districts, pr20_district_kg$DISTRICT_NAME),]
pr20_district_kg <- pr20_district_kg[, c("DISTRICT_NAME", "COPPER ROCKFISH", "BLACK ROCKFISH", "CANARY ROCKFISH")]

genus_allocate[1:6, 4:6] <- pc20_district_kg[1:6, 2:4]
genus_allocate[13:18, 4:6] <- pc21_district_kg[1:6, 2:4]
genus_allocate[25:30, 4:6] <- pr20_district_kg[1:6, 2:4]

write.csv(genus_allocate, "genus_allocate_20230602.csv")
